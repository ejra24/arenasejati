...
// DAFTAR KELAS AKTIF (KHUSUS HALAMAN KACER - BERDASARKAN BROSUR)
$kelas_kacer = [
    2  => ["nama" => "Sesi 2 - Kacer Tilas", "harga" => 55000],
    8  => ["nama" => "Sesi 8 - Kacer Marwah", "harga" => 125000],
    17 => ["nama" => "Sesi 17 - Kacer Napak", "harga" => 85000],
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_booking'])) {
    ...
    $harga_satuan = $kelas_kacer[$id_kelas_pilih]['harga'] ?? 55000;
    ...
    $merchantOrderId    = 'KACER-' . $id_transaksi_lokal . '-' . time();
    ...
    $params = [
        ...
        'returnUrl'       => 'http://127.0.0.1/kasirBurung/kacer.php',
        'expiryPeriod'    => 10,
        'signature'       => $signature
    ];
...
if (isset($_GET['get_status_kelas'])) {
    ...
    if (!array_key_exists($id_req, $kelas_kacer)) {
        echo json_encode(['error' => 'Kelas tidak ditemukan']);
        exit;
    }
    
    $nama_kelas_req = $kelas_kacer[$id_req]['nama'];
    ...
    echo json_encode([
        'harga' => $kelas_kacer[$id_req]['harga'],
...
$id_awal = 2;
$nama_kelas_awal = $kelas_kacer[$id_awal]['nama'];
$harga_awal = $kelas_kacer[$id_awal]['harga'];
...
<div class="main-wrapper" style="margin-top: 100px;">
    <div class="left-column">
        ...
        <div class="seat-section">
            ...
            <h2>Area Pemilihan Gantangan - Kacer</h2>
            ...
            <div class="select-kelas-box">
                ...
                <select id="id_kelas_pilih" onchange="muatUlangKomponenKelas(this.value)">
                    <?php foreach($kelas_kacer as $id => $item): ?>
        ...
        <!-- HANYA ADMIN YANG DAPAT MELIHAT DATA REGISTRASI INI -->
        <?php if ($is_admin): ?>
...
<script>
    ...
    // Elemen registrasi opsional jika admin
    ...
    const daftarNamaKelas = <?php
        $js_array = [];
        foreach($kelas_kacer as $id => $item) { $js_array[$id] = $item['nama']; }
    ...
    function muatUlangKomponenKelas(idKelas) {
        ...
        .then(data => {
            ...
            // Render ulang bagian registrasi HANYA jika admin
            if (IS_ADMIN && registrationSection && listContainer) {
