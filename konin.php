...
// DAFTAR KELAS AKTIF (KHUSUS HALAMAN KONIN - BERDASARKAN BROSUR)
$kelas_konin = [
    6  => ["nama" => "Sesi 6 - Konin Tilas", "harga" => 55000],
    12 => ["nama" => "Sesi 12 - Konin Marwah", "harga" => 125000],
    21 => ["nama" => "Sesi 21 - Konin Napak", "harga" => 85000]
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_booking'])) {
    ...
    $harga_satuan = $kelas_konin[$id_kelas_pilih]['harga'] ?? 55000;
    ...
    $merchantOrderId    = 'KONIN-' . $id_transaksi_lokal . '-' . time();
    ...
    $params = [
        ...
        'returnUrl'       => 'http://127.0.0.1/kasirBurung/konin.php',
        'expiryPeriod'    => 10,
        'signature'       => $signature
    ];
...
// ---------------------------------------------------------------------------
// BAGIAN 1B: PROSES HAPUS / BATAL REGISTRASI
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// BAGIAN 2: AJAX ENDPOINT UNTUK AMBIL DATA KELAS & KURSI
// ---------------------------------------------------------------------------
if (isset($_GET['get_status_kelas'])) {
    header('Content-Type: application/json');
    $id_req = (int)$_GET['get_status_kelas'];
    
    if (!array_key_exists($id_req, $kelas_konin)) {
        echo json_encode(['error' => 'Kelas tidak ditemukan']);
        exit;
    }
    
    $nama_kelas_req = $kelas_konin[$id_req]['nama'];
    
    // Hanya ambil yang tidak batal
    ...
    echo json_encode([
        'harga' => $kelas_konin[$id_req]['harga'],
...
// Data awal saat pertama kali halaman diakses (Sesi Konin Pertama: Sesi 6)
$id_awal = 6;
$nama_kelas_awal = $kelas_konin[$id_awal]['nama'];
$harga_awal = $kelas_konin[$id_awal]['harga'];
...
<div class="main-wrapper" style="margin-top: 100px;">
    <div class="left-column">
        ...
        <div class="seat-section">
            ...
            <h2>Area Pemilihan Gantangan - Konin</h2>
            ...
            <div class="select-kelas-box">
                ...
                <select id="id_kelas_pilih" onchange="muatUlangKomponenKelas(this.value)">
                    <?php foreach($kelas_konin as $id => $item): ?>
        ...
        <!-- KOTAK REGISTRASI PESERTA: Hanya Muncul Jika yang Login adalah ADMIN -->
        <?php if ($is_admin): ?>
                    ...
                    <div class="reg-card">
                        <div class="reg-info">
                            ...
                            <div style="margin-top:8px;">
                                <button onclick="handleDelete(this)" data-id="<?php echo $reg['id']; ?>" class="btn-primary" style="display:inline-block;padding:6px 10px;font-size:12px;background:#e11d48;border-color:#e11d48; width:auto; margin-top:0;">Hapus</button>
                            </div>
                        </div>
                        <div class="reg-seat">
        ...
        <div class="floating-card">
            ...
            <form id="form-transaksi">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="alamat" id="input-alamat" placeholder="Masukkan alamat / tim" required>
                </div>
...
<script>
    ...
    const daftarNamaKelas = <?php
        $js_array = [];
        foreach($kelas_konin as $id => $item) { $js_array[$id] = $item['nama']; }
    ...
    function muatUlangKomponenKelas(idKelas) {
        ...
        .then(data => {
            ...
            // Jika bukan admin, abaikan pembaruan list registrasi
            if (!isAdmin || !listContainer) return;
