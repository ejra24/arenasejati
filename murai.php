...
// Daftar Kelas Khusus Murai Borneo sesuai brosur Kapolresta Samarinda Cup
$kelas_murai = [
    1  => ["nama" => "Sesi 1 - Murai Borneo Tilas", "harga" => 55000],
    7  => ["nama" => "Sesi 7 - Murai Borneo Marwah", "harga" => 125000],
    13 => ["nama" => "Sesi 13 - Murai Borneo Sejati", "harga" => 560000],
    16 => ["nama" => "Sesi 16 - Murai Borneo Sultan", "harga" => 220000],
    22 => ["nama" => "Sesi 22 - Murai Borneo Napak", "harga" => 85000],
];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_booking'])) {
    ...
    $harga_satuan = $kelas_murai[$id_kelas_pilih]['harga'] ?? 55000;
    ...
    $merchantOrderId    = 'MURAI-' . $id_transaksi_lokal . '-' . time();
    ...
    $params = [
        ...
        'returnUrl'       => 'http://127.0.0.1/kasirBurung/murai.php',
        'expiryPeriod'    => 10,
        'signature'       => $signature
    ];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action_delete'])) {
    ...
    if ($q) {
        echo json_encode(['status' => 'success', 'message' => 'Registrasi dihapus']);
    } else {
...
// ---------------------------------------------------------------------------
// BAGIAN 2: AJAX ENDPOINT (Get Data Kelas)
// ---------------------------------------------------------------------------
if (isset($_GET['get_status_kelas'])) {
    header('Content-Type: application/json');
    $id_req = (int)$_GET['get_status_kelas'];
    
    if (!array_key_exists($id_req, $kelas_murai)) { echo json_encode(['error' => 'Kelas tidak ditemukan']); exit; }
    
    $nama_kelas_req = $kelas_murai[$id_req]['nama'];
    ...
    echo json_encode([
        'harga' => $kelas_murai[$id_req]['harga'],
...
// Data awal saat pertama kali halaman diakses (Default Kelas ID: 1)
$id_awal = 1;
$nama_kelas_awal = $kelas_murai[$id_awal]['nama'];
$harga_awal = $kelas_murai[$id_awal]['harga'];
...
<div class="main-wrapper" style="margin-top: 100px;">
    <div class="left-column">
        <div class="seat-section">
            ...
            <h2>Area Pemilihan Gantangan - Murai Borneo</h2>
            ...
            <div class="select-kelas-box">
                ...
                <select id="id_kelas_pilih" onchange="muatUlangKomponenKelas(this.value)">
                    <?php foreach($kelas_murai as $id => $item): ?>
...
<script>
    ...
    const daftarNamaKelas = <?php 
        $js_array = [];
        foreach($kelas_murai as $id => $item) { $js_array[$id] = $item['nama']; }
    ...
    function renderGantanganMap() {
        ...
        for (let i = 1; i <= TOTAL_GANTANGAN; i++) {
            ...
            if (kursiLunas.includes(i)) {
                ...
                if (kursiTerpilihArr.includes(i)) {
                    divKursi.classList.add('selected');
                }
    ...
    function handleDelete(btnEl) {
        ...
        .then(res => {
            if (res.status === 'success') {
                alert(res.message);
                muatUlangKomponenKelas(hiddenIdKelas.value);
            } else {
    ...
    document.getElementById('form-transaksi').addEventListener('submit', function(e) {
        e.preventDefault();
        if (kursiTerpilihArr.length === 0) {
            alert('Silakan pilih minimal 1 gantangan terlebih dahulu!');
            return;
        }
